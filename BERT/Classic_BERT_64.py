...
def start(jira_name, main_path):
    ...
    for k_unstable in [5,10,15,20]:
        ...
        if jira_name == 'Nothing':
            model = TFBertForSequenceClassification.from_pretrained(f'YakovElm/{jira_name}{k_unstable}Classic_64')
            ...
            model.push_to_hub(f"YakovElm/{jira_name}{k_unstable}Classic_64")
        ...
        print('start to save')
        results.to_csv(f'{main_path}BERT/Results/{jira_name}/Classic_64_result_{k_unstable}.csv', index=False)
        print('ended to save')
...
def add_new_threshold(results, model, jira_name, main_path, k_unstable, predictions, test_input_ids,
    ...
    accuracy, confusion_matrix, classific_report, area_under_pre_recall_curve, average_precision, auc, \
        ...
        get_report(model=model, kind_of_bert="Classic_64", jira_name=jira_name, main_path=main_path,
