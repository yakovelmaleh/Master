...
import DoubleCheck.ml_algorithms_run_best_parameters as ml_algorithms_run_best_parameters
import pandas as pd
...
def start(jira_name):
    ...
    for label_name in dict_labels.items():
        ...
        path = addPath(f'Master/DoubleCheck/Results/{project_key}')
        results.to_csv(f'{path}/results_{project_key}_{label_name[0]}_DoubleCheck.csv', index=False)
