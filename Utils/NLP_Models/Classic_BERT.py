...
def start(jira_name, model_name, train_data_list, valid_data_list, test_data_list, path_to_save):
    ...
    for k_unstable in [5, 10, 15, 20]:
        ...
        valid = valid_data_list[f'{k_unstable}']
        ...
        model.push_to_hub(f"YakovElm/{jira_name}_{k_unstable}_BERT_{model_name}")

        # Assuming you have predictions for the test data
        predictions = model.predict([test_input_ids, test_attention_mask])
        predictions = predictions[0]

        y_score = softmax(predictions, axis=1)
        y_pred = np.where(y_score[:, 1] >= 0.998, 1, 0)

        accuracy, confusion_matrix, classification_report, area_under_pre_recall_curve, average_precision, auc =\
            Get_Results.get_results(y_score=y_score, y_pred=y_pred, y_test=test_labels,
                                    model_name=f'{jira_name}_{k_unstable}_BERT_{model_name}', path_to_save=path_to_save)
    ...
    results.to_csv(
        f'{path_to_save}/Results_BERT_{jira_name}_{model_name}.csv', index=False)
