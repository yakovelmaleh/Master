...
def run_model_optimization(x_train, x_test, y_train, y_test, project_key, label, all_but_one_group, path):
    ...
    accuracy_rf, confusion_matrix_rf, classification_report_rf, \
        ...
        feature_importance, best_params = run_best_params_CV(model=RandomForestClassifier(),dict=parmas, model_name="RF",
    ...
    d = {'project_key': project_key, 'usability_label': label,
         ...
         'max_iterations': best_params['max_iter'], 'solver': best_params['solver'], 'num_batches_size': best_params['batch_size'],
         'activation': best_params['activation'],'alpha': best_params['alpha'], 'learning_rate': best_params['learning_rate'] }
...
def run_best_params_CV(model, dict, model_name, x_train, x_test, y_train, y_test, project_key,label,all_but_one_group):
    """
    ...
    area_under_pre_recall_curve = create_pre_rec_curve(y_test, y_score, average_precision,project_key=project_key,
                                                       label=label,all_but_one_group=all_but_one_group, algorithm=model_name)
