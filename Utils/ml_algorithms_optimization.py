...
def run_random_forest(x_train, x_test, y_train, y_test, num_trees, max_feature, max_depths, min_sample_split,
    ...
    area_under_pre_recall_curve = create_pre_rec_curve(y_test, y_score, average_precision,project_key=project_key,label=label,all_but_one_group=all_but_one_group, algorithm="RF" )
...
def run_neural_net(x_train, x_test, y_train, y_test, num_unit_hidden_layer, max_iteration, solvers, num_batch, activations,project_key,label,all_but_one_group):
    """
    ...
    print(8)
    area_under_pre_recall_curve = create_pre_rec_curve(y_test, y_score, average_precision,project_key=project_key,label=label,all_but_one_group=all_but_one_group,algorithm="NN")
...
def run_xgboost(x_train, x_test, y_train, y_test, num_trees, max_feature, max_depths, alpha_t, scale_pos,
    ...
    area_under_pre_recall_curve = create_pre_rec_curve(y_test, y_score, average_precision,project_key=project_key, label=label,all_but_one_group=all_but_one_group, algorithm="XG")
                        ...
                        for bootstrap in bootstraps:
                            accuracy_rf, confusion_matrix_rf, classification_report_rf, \
                                ...
                                feature_importance = run_random_forest(x_train, x_test, y_train,
                                                                        y_test, num_trees, max_feature, max_depth,
                                                                   min_sample_split, min_sample_leaf, bootstrap, project_key=project_key, label=label, all_but_one_group=all_but_one_group)
                            d = {'project_key': project_key, 'usability_label': label, 'features': features,
                ...
                for min_sample_split in min_samples_split:
                    for min_sample_leaf in min_samples_leaf:
                        accuracy_xgboost, confusion_matrix_xgboost, classification_report_xgboost, \
                            ...
                            y_pred_xgboost, = run_xgboost(x_train, x_test, y_train, y_test, num_trees, max_feature, max_depth,
                                                               min_sample_split, min_sample_leaf,project_key=project_key, label=label,all_but_one_group=all_but_one_group)
                ...
                for num_batch in num_batches_size:
                    for activation in activations:
                        accuracy_nn, confusion_matrix_nn, classification_report_nn, \
                            ...
                            y_pred_nn = run_neural_net(x_train_nn, x_test_nn, y_train,
                                                       y_test, num_unit_hidden_layer, max_iteration,
                                                       solver, num_batch, activation, project_key=project_key, label=label, all_but_one_group=all_but_one_group)
                        d = {'project_key': project_key, 'usability_label': label,
