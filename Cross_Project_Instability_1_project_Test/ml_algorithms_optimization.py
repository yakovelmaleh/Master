...
def create_pre_rec_curve(y_test, y_score, average_precision, label, all_but_one_group, algorithm):
    ...
    if all_but_one_group:
        ...
        plt.savefig(
            f'{path}/pre_recall_curve_{label}_{algorithm}_2.png')
    plt.close()
    return area


def run_model_optimization(x_train, x_test, y_train, y_test, label, all_but_one_group):
    print(list(x_train))
    ...
    rf_results.to_csv(
        f'{path}/results_groups_label_{label}_RF.csv', index=False)
    ...
    xgboost_results.to_csv(
        f'{path}/results_groups_label_{label}_XGboost.csv', index=False)
    ...
    path = addPath(f'Master/Cross_Project_Instability_1_project_Test/Parameters')
    nn_results.to_csv(
        f'{path}/results_groups_label_{label}_NN.csv', index=False)
