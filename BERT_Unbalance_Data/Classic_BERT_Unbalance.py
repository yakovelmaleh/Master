    ...
    for k_unstable in [5,10,15,20]:
        ...
        if jira_name == 'Nothings':
            model = TFBertForSequenceClassification.\
                from_pretrained(f'YakovElm/{jira_name}{k_unstable}Classic_Unbalance')
            ...
            optimizer = tf.keras.optimizers.Adam(learning_rate=learning_rate, epsilon=epsilon, clipnorm=clipnorm, weight_decay=weight_decay)
            ...
            model.push_to_hub(f"YakovElm/{jira_name}{k_unstable}Classic_Unbalance")
        ...
        print('start to save')
        results.to_csv(f'{main_path}BERT_Unbalance_Data/Results/{jira_name}/Classic_result_{k_unstable}_Unbalance.csv',
                       index=False)
        print('ended to save')
