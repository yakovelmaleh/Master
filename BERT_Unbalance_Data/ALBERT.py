...
def get_report(model, kind_of_bert, jira_name, main_path, k_unstable, test_predictions, x_test, test_labels,
               threshold=None):
    if kind_of_bert == 'Classic' or kind_of_bert == 'ALBERT':
    ...
    if threshold is None or threshold == 0.5:
        path = f'{main_path}BERT_Unbalance_Data/Results/{jira_name}'
...
def start(jira_name, main_path):
    ...
    for k_unstable in [5, 10, 15, 20]:
        ...
        if jira_name == 'Nothings':
            model = TFAutoModelForSequenceClassification. \
                from_pretrained(f'YakovElm/{jira_name}{k_unstable}ALBERT_Unbalance')

        else:
            model = TFAutoModelForSequenceClassification.from_pretrained(model_name)
            train, valid = get_data_train_with_labels(jira_name, main_path, k_unstable)
            ...
            # Modify the BERT model's config to include dropout
            ...
            model.push_to_hub(f"YakovElm/{jira_name}{k_unstable}ALBERT_Unbalance")
        ...
        print('start to save')
        results.to_csv(f'{main_path}BERT_Unbalance_Data/Results/{jira_name}/ALBERT_result_{k_unstable}_Unbalance.csv',
                       index=False)
        print('ended to save')
...
def add_new_threshold(results, model, jira_name, main_path, k_unstable, predictions, test_input_ids,
    ...
    accuracy, confusion_matrix, classific_report, area_under_pre_recall_curve, average_precision, auc, \
        ...
        get_report(model=model, kind_of_bert="ALBERT", jira_name=jira_name, main_path=main_path,
