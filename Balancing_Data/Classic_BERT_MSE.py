...
def get_report(model, kind_of_bert, jira_name, main_path, k_unstable, test_predictions, x_test, test_labels,
    ...
    if threshold is None or threshold == 0.5:
        path = f'{main_path}Balancing_Data/Results/{jira_name}'
...
def start(jira_name, main_path):
    ...
    for k_unstable in [5,10,15,20]:
        ...
        if jira_name == 'Apache' or jira_name == 'Hyperledger' or jira_name == 'IntelDAOS' or \
            ...
            model = TFBertForSequenceClassification.from_pretrained(f'YakovElm/{jira_name}{k_unstable}Classic_MSE')
            ...
            model.push_to_hub(f"YakovElm/{jira_name}{k_unstable}Classic_MSE")
        ...
        print('start to save')
        print(f'path: {main_path}Balancing_Data/Results/{jira_name}/Classic_result_{k_unstable}.csv')
        results.to_csv(f'{main_path}Balancing_Data/Results/{jira_name}/Classic_result_{k_unstable}.csv', index=False)
        print('ended to save')
...
def add_new_threshold(results, model, jira_name, main_path, k_unstable, predictions, test_input_ids,
    ...
    accuracy, confusion_matrix, classific_report, area_under_pre_recall_curve, average_precision, auc, \
        ...
        get_report(model=model, kind_of_bert="Classic_MSE", jira_name=jira_name, main_path=main_path,
