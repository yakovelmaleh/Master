...
def get_report(model, kind_of_bert, jira_name, main_path, k_unstable, test_predictions, x_test, test_labels,
    ...
    if threshold is None or threshold == 0.5:
        path = f'{main_path}Balance_only_Train/Results/{jira_name}'
...
def get_balance_data(jira_name, main_path, k_unstable, ratio):
    train_data = GetNLPData.get_data_train(jira_name, main_path, k_unstable)
    ...
    train, validation = train_test_split(shuffled_all_data, test_size=0.27, random_state=42)

    test = GetNLPData.get_test_data(jira_name, main_path, k_unstable)
...
def start(jira_name, main_path):
    ...
    for k_unstable in [5, 10, 15, 20]:
        ...
        for ratio in ratio_lst:
            ...
            trainer.push_to_hub(f"YakovElm/{jira_name}{k_unstable}SetFitModel_Train_balance_ratio_{ratio_name}")
            ...
            accuracy, confusion_matrix, classification_report, area_under_pre_recall_curve, average_precision, auc, \
                ...
                get_report(model=model, kind_of_bert=f"SetFit_Train_ratio_{ratio_name}", jira_name=jira_name,
                           main_path=main_path, k_unstable=k_unstable, test_predictions=test_predictions,
                           x_test=test_sentence, test_labels=test_labels)
        ...
        results.to_csv(f'{main_path}Balance_only_Train/'
