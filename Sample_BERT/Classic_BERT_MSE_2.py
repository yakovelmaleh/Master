...
def create_pre_rec_curve(y_test, y_score, auc, algorithm, jira_name, label, path):
    ...
    plt.savefig(f'{path}/pre_recall_curve_groups_{jira_name}_{label}_{algorithm}_MSE_2.png')
    plt.close()
    return area
    ...
    for k_unstable in [5,10,15,20]:
        ...
        if jira_name == 'Nothing':
            model = TFBertForSequenceClassification.from_pretrained(
                f'YakovElm/{jira_name}{k_unstable}Classic_MSE_Sample_2')
            ...
            model.push_to_hub(f"YakovElm/{jira_name}{k_unstable}Classic_MSE_Sample_2")
        ...
        print('start to save')
        results.to_csv(f'{main_path}Sample_BERT/Results/{jira_name}/Classic_result_{k_unstable}_MSE_2.csv', index=False)
        print('ended to save')
...
def add_new_threshold(results, model, jira_name, main_path, k_unstable, predictions, test_input_ids,
    ...
    accuracy, confusion_matrix, classific_report, area_under_pre_recall_curve, average_precision, auc, \
        ...
        get_report(model=model, kind_of_bert="Classic_MSE_2", jira_name=jira_name, main_path=main_path,
