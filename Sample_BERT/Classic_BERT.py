...
def compute_class_weights(labels):
    ...
    inv_class_freq = 1.0 / class_counts

    # Normalize the class weights
    class_weights = inv_class_freq / np.sum(inv_class_freq)
    print(class_weights)
...
def create_pre_rec_curve(y_test, y_score, auc, algorithm, jira_name, label, path):
    ...
    plt.savefig(f'{path}/pre_recall_curve_groups_{jira_name}_{label}_{algorithm}_Cross_entropy.png')
    plt.close()
    return area
...
def get_report(model, kind_of_bert, jira_name, main_path, k_unstable, test_predictions, x_test, test_labels,
    ...
    if threshold is None or threshold == 0.5:
        path = f'{main_path}Sample_BERT/Results/{jira_name}'
    ...
    for k_unstable in [5,10,15,20]:
        ...
        if jira_name == 'Nothing':
            model = TFBertForSequenceClassification.from_pretrained(
                f'YakovElm/{jira_name}{k_unstable}Classic_Cross_entropy_Sample')

        else:
            model = TFBertForSequenceClassification.from_pretrained('bert-base-uncased')
            train, valid = GetNLPData.get_data_train_with_labels(jira_name, main_path, k_unstable)
            ...
            model.push_to_hub(f"YakovElm/{jira_name}{k_unstable}Classic_Cross_entropy_Sample")
        ...
        results = add_new_threshold(results, model, jira_name, main_path, k_unstable, predictions, test_input_ids,
                                    test_attention_mask, test_labels, 0.998)

        print('start to save')
        print(f'path: {main_path}Sample_BERT/Results/{jira_name}/Classic_result_{k_unstable}.csv')
        results.to_csv(f'{main_path}Sample_BERT/Results/{jira_name}/Classic_result_{k_unstable}_Cross_entropy.csv', index=False)
        print('ended to save')
...
def add_new_threshold(results, model, jira_name, main_path, k_unstable, predictions, test_input_ids,
    ...
    accuracy, confusion_matrix, classific_report, area_under_pre_recall_curve, average_precision, auc, \
        ...
        get_report(model=model, kind_of_bert="Classic_Cross_entropy_Sample", jira_name=jira_name, main_path=main_path,
