...
def create_pre_rec_curve(y_test, y_score, auc, algorithm, label, path):
    ...
    plt.savefig(f'{path}/pre_recall_curve_groups_{label}_{algorithm}.png')
    plt.close()
    return area
...
def start():
    ...
    for k_unstable in [5, 10, 15, 20]:
        ...
        if flag:
            model = TFBertForSequenceClassification.from_pretrained(f"YakovElm/Cross_Project_{k_unstable}"
                                                                    f"_Classic_without_validation")
            ...
            model.push_to_hub(f"YakovElm/Cross_Project_{k_unstable}_Classic_without_validation")
        ...
        results.to_csv(f'{main_path}Cross_Projects/Results/Cross_Projects_Classic_result_without_validation'
                       f'_{k_unstable}.csv', index=False)
        results = pd.DataFrame(columns=['usability_label', 'threshold', 'accuracy',
                                        'confusion_matrix', 'classification_report', 'area_under_pre_recall_curve',
                                        'avg_precision', 'area_under_roc_curve', 'y_pred', 'precision',
                                        'recall', 'thresholds'])
...
def add_new_threshold(results, main_path, k_unstable, predictions, test_labels, threshold):
    accuracy, confusion_matrix, classific_report, area_under_pre_recall_curve, average_precision, auc, \
        ...
        get_report(kind_of_bert="Classic_without_validation", main_path=main_path, k_unstable=k_unstable,
                   test_predictions=predictions, test_labels=test_labels, threshold=threshold)
