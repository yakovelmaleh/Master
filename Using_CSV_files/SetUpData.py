...
def get_sprint_info(auth_jira, sql_sprints, issue_s, name_map):
    """
    the function return and save the issue sprint information
        ...
        sprints.
    """
    project_key = issue_s.fields.project.key
    issue_key = issue_s.key
    ...
    if sprint is not None:
        for i in range(0, len(sprint)):
            ...
            is_over = 1
            if end_date1[1] == 'n':
                start_date = create_date_from_string(issue_s.fields.created)
                end_date = create_date_from_string(
                    issue_s.fields.resolutiondate) if issue_s.fields.resolutiondate is not None else (
                    create_date_from_string(issue_s.fields.updated) if issue_s.fields.updated is not None else None)
                is_over = 0
            else:
            ...
            try:
                sprints.append(
                    (issue_key, project_key, sprint[i].split("name=")[1].split(",", 1)[0], start_date, end_date,
                     is_over, i + 1))
            except:
    ...
    another_way_to_get_sprints = get_Sprints_From_History(auth_jira, issue_s)

    sprints = sprints if len(sprints) >= len(another_way_to_get_sprints) else another_way_to_get_sprints

    if len(sprints) != 0:
        print(sprints)
        a = TableColumns.sprints_os.copy()
        # commit_list_of_queries(None, sql_sprints, sprints)
        ...
        for i in range(0, len(histories)):
            for j in range(0, len(histories[i].items)):
                ...
                if field == 'Sprint':
                    ...
                    row = [issue_key, project_key, to_string, created, end_date, 0, index]
                    if len(sprint_output) > 0:
                        previous_row = sprint_output.pop()
                        previous_row = [issue_key, project_key, previous_row[2], previous_row[3],
                                        created, 0, index - 1]
                        sprint_output.append(previous_row)
                    elif from_string is not None and from_string != "":
                        previous_row = [issue_key, project_key, from_string,
                                        create_date_from_string(issue.fields.created),
                                        created, 0, index - 1]
                        sprint_output.append(previous_row)
    ...
    for project_name in auth_jira.projects():
        ...
        while True:
            ...
            *************** run over all issues****************
            ...
            """
            # for testing:
            issues = auth_jira.search_issues('key = USERGRID-1240 ')
            ""
            for issue in issues:
                try:
                    time.sleep(1)
                    ...
                    num_sprints = get_sprint_info(auth_jira, sql_sprints, issue, name_map)
                    if num_sprints == 0:
                        continue
