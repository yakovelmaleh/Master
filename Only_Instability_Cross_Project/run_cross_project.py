...
def get_cross_project_train_data_except(irrelevant_projects):
    output = dict()
    for k_unstable in [5, 10, 15, 20]:
        output[f'{k_unstable}'] = GetInstabilityData. \
            get_all_data_except('Master/', k_unstable, irrelevant_projects)

    return output


def get_cross_project_labels_except(irrelevant_projects):
    output = dict()
    for k_unstable in [5, 10, 15, 20]:
        output[f'{k_unstable}'] = GetInstabilityData. \
            get_all_label_except('Master/', k_unstable, irrelevant_projects)

    return output


def get_all_train_data_project(project_name):
    output = dict()
    for k_unstable in [5, 10, 15, 20]:
        output[f'{k_unstable}'] = \
            GetInstabilityData.get_all_data_project(project_name, 'Master/', k_unstable)

    return output


def get_all_labels_project(project_name):
    output = dict()
    for k_unstable in [5, 10, 15, 20]:
        output[f'{k_unstable}'] = \
            GetInstabilityData.get_all_label_project(project_name, 'Master/', k_unstable)
