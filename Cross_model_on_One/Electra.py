...
def start(jira_name):
    ...
    for k_unstable in [5, 10, 15, 20]:
        ...
        # save model
        model.push_to_hub(f"YakovElm/{jira_name}{k_unstable}_ElectraModel_full")

        # Evaluation loop (assuming you have a separate evaluation dataset)
        data = GetNLPData.get_data_all_train_except(jira_name, 'Master/', k_unstable)
    ...
    path = f'Master/Cross_model_on_One/Results'
    results.to_csv(
