...
def create_pre_rec_curve(y_test, y_score, auc, algorithm, label, all_but_one_group, auc_PRC_path=None):
    """
...
def run_XG(x_train, x_test, y_train, y_test, num_trees, max_depth_xg,
           max_features, min_samples_split, min_samples_leaf, learning_rate, subsample, random_state, label,
           all_but_one_group, auc_PRC_path=None):
    x_train = x_train.copy()
    x_test = x_test.copy()
    ...
    return run_generic_model_with_sample_weight(clf, "XGboost", x_train, x_test, y_train, y_test, label,
                                                all_but_one_group, auc_PRC_path)
...
def run_NN(x_train, x_test, y_train, y_test, solver_nn, alpha_nn,
           ...
           label, all_but_one_group, auc_PRC_path=None):
    x_train = x_train.drop(columns=['created'])
    x_test = x_test.drop(columns=['created'])
    x_train = x_train.drop(columns=['issue_key'])
    x_test = x_test.drop(columns=['issue_key'])
    ...
    return run_generic_model(clf, "NN", X_train_resampled, x_test_nn1, y_train_resampled,
                             y_test, label, all_but_one_group, auc_PRC_path)


def run_generic_model(clf, model_name, x_train, x_test, y_train, y_test, label, all_but_one_group, auc_PRC_path=None):
    ...
    create_pre_rec_curve(y_test, y_score[:, 1], average_precision, model_name, label, all_but_one_group, auc_PRC_path)
...
def run_generic_model_with_sample_weight(clf, model_name, x_train, x_test,
                                         y_train, y_test, label, all_but_one_group, auc_PRC_path=None):
    class_weights = dict(
    ...
    create_pre_rec_curve2(y_test, y_score[:, 1], sample_weight,
                          average_precision, model_name, label, all_but_one_group, auc_PRC_path)

    return [accuracy, confusion_matrix, classification_report, area_under_pre_recall_curve, average_precision, auc,
            y_pred, feature_imp, precision, recall, thresholds]


def create_pre_rec_curve2(y_test, y_score, sample_weight, auc, algorithm, label, all_but_one_group, auc_PRC_path=None):
    """
    this function create the precision and recall curve and save the fig results
    """
